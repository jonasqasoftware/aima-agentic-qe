...
export function formatMarkdown(report) {
  ...
  const comparison = report.baselineComparison
    ? `\n## Comparação com baseline\n\n> ${report.baselineComparison.boundary}\n\n- **Baseline:** \`${report.baselineComparison.baselineChangeId}\` (${report.baselineComparison.baselineRecommendation})\n- **Variação de Quality Confidence:** ${report.baselineComparison.qualityConfidenceComparable ? `${report.baselineComparison.qualityConfidenceDelta >= 0 ? '+' : ''}${report.baselineComparison.qualityConfidenceDelta}` : `${report.baselineComparison.qualityConfidenceDelta >= 0 ? '+' : ''}${report.baselineComparison.qualityConfidenceDelta} (modelos diferentes — não representa tendência comparável)`}\n- **Riscos novos:** ${report.baselineComparison.newRisks.map((risk) => risk.id).join(', ') || 'nenhum'}\n- **Riscos resolvidos:** ${report.baselineComparison.resolvedRisks.map((risk) => risk.id).join(', ') || 'nenhum'}\n- **Riscos com score alterado:** ${report.baselineComparison.changedRisks.map((risk) => `${risk.id} (${risk.previousScore}→${risk.currentScore})`).join(', ') || 'nenhum'}\n`
    : '';
  const confidenceModel = report.qualityConfidence.model
    ? `\n- **Modelo:** \`${report.qualityConfidence.model.id}\` · v${report.qualityConfidence.model.version}`
    : '';
  const confidenceCalculation = report.qualityConfidence.calculation
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:radial-gradient(circle at top right,#183257,transparent 36%),var(--bg); color:var(--ink); font:16px/1.5 Inter,ui-sans-serif,system-ui,sans-serif; }
    ...
    .REMOTE_EVIDENCE { color:#b8c7ff; background:#242c64; } .EXECUTED_EVIDENCE { color:#ffb7ea; background:#54204b; } .TEST_RESULTS_EVIDENCE { color:#ffcf87; background:#59380e; } .COVERAGE_EVIDENCE { color:#8ee6ff; background:#164654; } footer { margin-top:46px; color:var(--muted); font-size:.875rem; } @media (max-width:700px) { main { padding-top:32px; } .summary,.two-columns { grid-template-columns:1fr; } }
  </style>
</head>
<body><main>
  ...
  <section class="summary">
    <article class="panel decision"><div><p class="eyebrow">Recomendação de release</p><strong class="${recommendationClass}">${escapeHtml(report.strategy.recommendation)}</strong><p>${escapeHtml(report.strategy.rationale)}</p></div><p class="eyebrow">Framework · ${escapeHtml(report.framework.name)}</p><p class="eyebrow">Política · ${escapeHtml(report.strategy.policy.name)} · v${escapeHtml(report.strategy.policy.version)}${report.strategy.decisionReasonCode ? ` · ${escapeHtml(report.strategy.decisionReasonCode)}` : ''}</p></article>
    <article class="panel"><p class="eyebrow">Quality Confidence experimental${report.qualityConfidence.model ? ` · ${escapeHtml(report.qualityConfidence.model.id)} v${escapeHtml(report.qualityConfidence.model.version)}` : ''}</p><p class="confidence">${escapeHtml(report.qualityConfidence.score)}<span>/100</span></p>${report.qualityConfidence.factors.map((item) => `<p>${escapeHtml(item)}</p>`).join('')}${report.qualityConfidence.calculation ? `<p>Maior risco ${escapeHtml(report.qualityConfidence.calculation.highestRiskScore)}/100 → penalidade ${escapeHtml(report.qualityConfidence.calculation.riskPenalty)}; ${escapeHtml(report.qualityConfidence.calculation.unknownCount)} desconhecido(s) → penalidade ${escapeHtml(report.qualityConfidence.calculation.unknownPenalty)}.</p>` : ''}<p>Quality Confidence não é probabilidade, não aumenta com evidências positivas nesta versão e não aprova releases.</p></article>
  </section>
